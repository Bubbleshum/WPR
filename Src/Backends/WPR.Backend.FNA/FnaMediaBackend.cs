...
namespace WPR.Backend.FNA
{
    ...
    /// <see cref="FnaAudioBackend"/> and <see cref="FnaXactBackend"/>.</para>
    ///
    ...
    /// </summary>
    public sealed class FnaMediaBackend : IMediaBackend
    {
        #region Song playback (FAudio XNA_*)

        public void SongInit() => FAudio.XNA_SongInit();

        public void SongQuit() => FAudio.XNA_SongQuit();

        public float PlaySong(string fileName) => FAudio.XNA_PlaySong(fileName);

        public void PauseSong() => FAudio.XNA_PauseSong();

        public void ResumeSong() => FAudio.XNA_ResumeSong();

        public void StopSong() => FAudio.XNA_StopSong();

        public void SetSongVolume(float volume) => FAudio.XNA_SetSongVolume(volume);

        public bool GetSongEnded() => FAudio.XNA_GetSongEnded() != 0;

        public void EnableSongVisualization(bool enable) =>
            FAudio.XNA_EnableVisualization(enable ? 1u : 0u);

        public bool IsSongVisualizationEnabled() => FAudio.XNA_VisualizationEnabled() == 1;

        public void GetSongVisualizationData(float[] frequencies, float[] samples, int count) =>
            FAudio.XNA_GetSongVisualizationData(frequencies, samples, (uint) count);
