...
namespace WPR.Platform.Android.Audio
{
    ...
    /// <see cref="WPR.Backend.FNA.FnaMediaBackend"/> (Theorafile), which has no equivalent problem
    /// and which this class has no business duplicating.</para>
    ///
    ...
    /// </summary>
    public sealed class AndroidMediaBackend : IMediaBackend
    {
        /// <summary>
        /// The backend serving the game in this process, for <see cref="GameActivity"/> to reach on
        ...
        /// </summary>
        internal static AndroidMediaBackend? Current { get; private set; }

        /// <summary>Video half — Theorafile, untouched.</summary>
        private readonly WPR.Backend.FNA.FnaMediaBackend _video = new();

        public AndroidMediaBackend()
        {
            Current = this;
        }
        ...
        public float PlaySong(string fileName)
        {
            lock (_gate)
            {
                ...
                /* A new song is a fresh state: whatever the host suspended is gone. */
        ...
        #region Video decode — delegated to Theorafile

        public IntPtr OpenVideo(string fileName) => _video.OpenVideo(fileName);

        public void CloseVideo(ref IntPtr video) => _video.CloseVideo(ref video);
        ...
        public bool HasVideoStream(IntPtr video) => _video.HasVideoStream(video);

        public bool HasAudioStream(IntPtr video) => _video.HasAudioStream(video);
        ...
        public bool IsEndOfVideo(IntPtr video) => _video.IsEndOfVideo(video);
        ...
        public bool ReadVideoFrames(IntPtr video, IntPtr yuvBuffer, int frameCount) =>
            _video.ReadVideoFrames(video, yuvBuffer, frameCount);

        public int ReadVideoAudio(IntPtr video, IntPtr buffer, int length) =>
            _video.ReadVideoAudio(video, buffer, length);
