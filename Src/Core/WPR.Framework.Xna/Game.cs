...
{
	public class Game : IDisposable, WPR.Xna.Rhi.IGameLoopHost
	{
		...
		public bool IsMouseVisible
		{
			...
			set
			{
				if (INTERNAL_isMouseVisible != value)
				{
					...
					WPR.Xna.Rhi.XnaBackend.Platform.OnIsMouseVisibleChanged(value);
				}
			}
		}
		...
		/// <summary>
		/// WPR: the smallest fixed step we will honour — 1/60 s, which is also FNA's own default
		/// and the fastest frame any Windows Phone 7 device could present. A WP7 title asking for
		/// less is asking for something no phone it shipped on could do.
		///
		/// <para><b>Why this must be clamped rather than obeyed.</b> In fixed-timestep mode
		/// <c>Tick</c> sets <c>gameTime.ElapsedGameTime = TargetElapsedTime</c> verbatim, and WP7
		/// ports almost universally derive their delta as
		/// <c>gameTime.ElapsedGameTime.Milliseconds * 0.001f</c> — an <b>integer</b> millisecond
		/// read. Any target below 1 ms therefore hands every game on the machine a delta of
		/// exactly <b>zero</b>: nothing animates, no timer counts down, and any state machine that
		/// advances on elapsed time stops dead while the loop spins as fast as the CPU allows.</para>
		///
		/// <para>This is not hypothetical. Angry Birds' credits page (<c>bd::h</c>) sets
		/// <c>TargetElapsedTime = TimeSpan.FromTicks(3333)</c> — 0.33 ms — on entry, and restores
		/// the game's normal 333333 (30 fps) only from its <i>exit</i> handler <c>bd::b</c>. With
		/// the raw value honoured, the credits froze mid-scroll, the hidden golden egg's unlock
		/// animation never advanced to the frame that awards it, and no tap could leave the page,
		/// because the exit that would restore the frame rate is itself driven by the delta that
		/// is now zero. Unrecoverable without killing the game.</para>
		///
		/// <para>Games asking for 30 fps (333333) or 60 fps (166667) — i.e. all of them, in
		/// practice — are unaffected: the floor equals the 60 fps value.</para>
		/// </summary>
		private static readonly TimeSpan MinTargetElapsedTime = TimeSpan.FromTicks(166667);

		/// <summary>Log the clamp once per game, not once per set — a game that does this usually
		/// does it from its Update, i.e. every frame.</summary>
		private bool wprTargetElapsedClampLogged;
		...
		public TimeSpan TargetElapsedTime
		{
			...
			set
			{
				...
				if (value < MinTargetElapsedTime)
				{
					if (!wprTargetElapsedClampLogged)
					{
						wprTargetElapsedClampLogged = true;
						WprDebugTrace.WriteLine(
							"[wpr-trace] Game.TargetElapsedTime " + value.Ticks +
							" ticks is below one 60Hz frame; clamped to " +
							MinTargetElapsedTime.Ticks +
							" (see MinTargetElapsedTime — sub-millisecond steps make every " +
							"ElapsedGameTime.Milliseconds delta zero)"
						);
					}

					value = MinTargetElapsedTime;
				}
		...
		public Game()
		{
			...
			textInputControlDown = new bool[WPR.Xna.Rhi.XnaBackend.Platform.TextInputControlCharacterCount];
			...
			Window = WPR.Xna.Rhi.XnaBackend.Platform.CreateWindow();
		...
		protected virtual void Dispose(bool disposing)
		{
			if (!isDisposed)
			{
				if (disposing)
				{
					...
					if (Window != null)
					{
						WPR.Xna.Rhi.XnaBackend.Platform.DisposeWindow(Window);
					}
		...
		public void Tick()
		{
			...
			WPR.Xna.Rhi.XnaBackend.Platform.PollEvents(
				this,
				ref currentAdapter,
			...
			if (suppressDraw)
			{
				...
				// ...but a suppressed frame must still present if another thread is sitting
				// inside a GPU call. FNA3D's OpenGL driver defers off-thread resource calls to
				// the device thread and blocks the caller until SwapBuffers drains that queue,
				// so skipping the frame outright deadlocks any game that loads content on a
				// worker thread behind a static screen — a very common WP7 shape. Presenting
				// costs a blit of the unchanged backbuffer and drains the queue; the game's own
				// Draw stays suppressed, which is what it actually asked for.
				// See WPR.Xna.Rhi.OffThreadGpuCalls.
				if (WPR.Xna.Rhi.OffThreadGpuCalls.AnyInFlight)
				{
					try
					{
						if (BeginDraw())
					...
					{
						WprDebugTrace.WriteLine("[wpr-ex] Game.Tick off-thread GPU pump present threw: " + ex);
					}
				}
			}
			else
		...
		protected virtual bool ShowMissingRequirementMessage(Exception exception)
		{
			...
			{
				WPR.Xna.Rhi.XnaBackend.Platform.ShowRuntimeError(
					Window.Title,
		...
		{
			currentAdapter = WPR.Xna.Rhi.XnaBackend.Platform.RegisterGame(this);
			IsActive = true;

			// Perform initial check for a touch device
			TouchPanel.TouchDeviceExists = WPR.Xna.Rhi.XnaBackend.Platform.GetTouchCapabilities().IsConnected;
		}

		private void AfterLoop()
		{
			WPR.Xna.Rhi.XnaBackend.Platform.UnregisterGame(this);
		}
		...
		private void RunLoop()
		{
			 ...
			 * -caleb
			 */
			if (WPR.Xna.Rhi.XnaBackend.Platform.NeedsPlatformMainLoop())
			{
				/* This breaks control flow and jumps
				 * directly into the platform main loop.
				 * Nothing below this call will be executed.
				 */
				WPR.Xna.Rhi.XnaBackend.Platform.RunPlatformMainLoop(this);
			}
		...
		#region WPR.Xna.Rhi.IGameLoopHost

		/* Explicit implementation, deliberately. Three of these five members are internal on Game
		 * -- IsActive's setter, RedrawWindow and the RunApplication flag -- and they must stay
		 * that way: XNA 4.0 exposes Game.IsActive as get-only, and games bind this type's public
		 * surface by identity, so widening it would change the API WP7 titles compiled against.
		 * An explicit implementation is reachable only through the interface, which is exactly
		 * who needs it: the platform backend. Nothing about the public Game surface changes.
		 *
		 * Spine relocation step 1 (2026-09-01): the platform used to receive `Game` itself. It
		 * only ever touched these five members, so the seam names an interface instead and
		 * WPR.Framework.Xna no longer has to see FNA to declare it. */

		GameWindow WPR.Xna.Rhi.IGameLoopHost.Window => Window;

		Microsoft.Xna.Framework.Graphics.GraphicsDevice WPR.Xna.Rhi.IGameLoopHost.GraphicsDevice => GraphicsDevice;

		bool WPR.Xna.Rhi.IGameLoopHost.IsActive
		{
			get { return IsActive; }
			set { IsActive = value; }
		}

		bool WPR.Xna.Rhi.IGameLoopHost.RunApplication
		{
			get { return RunApplication; }
			set { RunApplication = value; }
		}

		void WPR.Xna.Rhi.IGameLoopHost.RedrawWindow()
		{
			RedrawWindow();
		}
