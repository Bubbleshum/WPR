	...
	public class GraphicsDevice : IDisposable
	{
		...
		public DisplayMode DisplayMode
		{
			get
			{
				...
				{
					int w, h;
					XnaBackend.Graphics.GetBackbufferSize(GLDevice, out w, out h);
					return new DisplayMode(
						w,
						h,
						XnaBackend.Graphics.GetBackbufferSurfaceFormat(GLDevice)
					);
				}
		...
		public Rectangle ScissorRectangle
		{
			...
			set
			{
				...
				XnaBackend.Graphics.SetScissorRect(
					GLDevice,
					ref value
				);
			}
		...
		public Color BlendFactor
		{
			...
			{
				Color result;
				XnaBackend.Graphics.GetBlendFactor(GLDevice, out result);
				return result;
			}
			set
			{
				 ...
				 * -flibit
				 */
				XnaBackend.Graphics.SetBlendFactor(GLDevice, ref value);
			}
		}
		...
		public int MultiSampleMask
		{
			get
			{
				return XnaBackend.Graphics.GetMultiSampleMask(GLDevice);
			}
			set
			{
				/* FIXME: Does this affect the value found in
				 ...
				 * -flibit
				 */
				XnaBackend.Graphics.SetMultiSampleMask(GLDevice, value);
			}
		}
		...
		public int ReferenceStencil
		{
			get
			{
				return XnaBackend.Graphics.GetReferenceStencil(GLDevice);
			}
			set
			{
				/* FIXME: Does this affect the value found in
				 ...
				 * -flibit
				 */
				XnaBackend.Graphics.SetReferenceStencil(GLDevice, value);
			}
		}
		...
		private RhiRenderTargetBinding[] nativeTargetBindings =
			new RhiRenderTargetBinding[MAX_RENDERTARGET_BINDINGS];
		private RhiRenderTargetBinding[] nativeTargetBindingsNext =
			new RhiRenderTargetBinding[MAX_RENDERTARGET_BINDINGS];
		...
		private readonly RhiVertexBufferBinding[] nativeBufferBindings =
			new RhiVertexBufferBinding[MAX_VERTEX_ATTRIBUTES];
...
#pragma warning restore 0067
		...
		public GraphicsDevice(
			...
			try
			{
				GLDevice = XnaBackend.Graphics.CreateDevice(
			...
			XnaBackend.Graphics.GetMaxTextureSlots(
				GLDevice,
				out maxTextures,
		...
		protected virtual void Dispose(bool disposing)
		{
			if (!IsDisposed)
			{
				if (disposing)
				{
					...
					XnaBackend.Graphics.DestroyDevice(GLDevice);

					/* WPR: drop our event subscribers.
					 *
					 * These are the LAST thing keeping a closed game's object graph alive, and
					 * therefore its collectible AssemblyLoadContext. A disposed GraphicsDevice can
					 * still be reachable from a static (the per-vertex-type VertexDeclaration cache
					 * holds VertexDeclarations, and every GraphicsResource holds its GraphicsDevice),
					 * and these events hold the GraphicsDeviceManager — which holds the game's Game
					 * subclass. Proven by gcroot on a heap dump:
					 *   static Object[] -> VertexDeclaration -> GraphicsDevice
					 *     -> EventHandler<EventArgs> -> GraphicsDeviceManager2 -> <game>.cMainGame
					 * Nulling them here severs that path; the device itself is WPR-owned and harmless
					 * if the cache outlives the launch. Done AFTER the Disposing notification above so
					 * subscribers still get their callback.
					 */
					DeviceLost = null;
					DeviceReset = null;
					DeviceResetting = null;
		...
		public void Reset(
			...
			PresentationParameters.MultiSampleCount = XnaBackend.Graphics.GetMaxMultiSampleCount(
				GLDevice,
		...
		private void ClearCore(ClearOptions options, ref Vector4 color, float depth, int stencil)
		{
			...
			if (renderTargetCount == 0)
			{
				 ...
				 * -flibit
				 */
				dsFormat = XnaBackend.Graphics.GetBackbufferDepthFormat(GLDevice);
			}
			else
			{
		...
		public void GetBackBufferData<T>(
			...
			if (rect == null)
			{
				x = 0;
				y = 0;
				XnaBackend.Graphics.GetBackbufferSize(
					GLDevice,
					out w,
					out h
				);
			...
			XnaBackend.Graphics.ReadBackbuffer(
				GLDevice,
				x,
				y,
				w,
		...
		public void SetRenderTargets(params RenderTargetBinding[] renderTargets)
		{
			...
			if (renderTargets == null || renderTargets.Length == 0)
			{
				...
				{
					XnaBackend.Graphics.ResolveTarget(GLDevice, ref nativeTargetBindings[i]);
				}
			...
			else
			{
				...
				unsafe
				{
					fixed (RhiRenderTargetBinding* rt = &nativeTargetBindingsNext[0])
					{
		...
		public void DrawIndexedPrimitives(
			...
			XnaBackend.Graphics.DrawIndexedPrimitives(
				GLDevice,
				primitiveType,
				baseVertex,
				minVertexIndex,
		...
		public void DrawInstancedPrimitives(
			...
			if (XnaBackend.Graphics.SupportsHardwareInstancing(GLDevice) == 0)
			{
			...
			XnaBackend.Graphics.DrawInstancedPrimitives(
				GLDevice,
				primitiveType,
				baseVertex,
				minVertexIndex,
		...
		public void DrawPrimitives(
			...
			XnaBackend.Graphics.DrawPrimitives(
				GLDevice,
				primitiveType,
				vertexStart,
				primitiveCount
		...
		{
			XnaBackend.Graphics.SetStringMarker(GLDevice, text);
		}
		...
		private void ApplyState()
		{
			...
			{
				XnaBackend.Graphics.SetBlendState(
					GLDevice,
					ref nextBlend.state
				);
			...
			{
				XnaBackend.Graphics.SetDepthStencilState(
					GLDevice,
			...
			XnaBackend.Graphics.ApplyRasterizerState(
				GLDevice,
		...
		private void ApplySamplers()
		{
			for (int sampler = 0; sampler < modifiedSamplers.Length; sampler += 1)
			{
				...
				try
				{
					XnaBackend.Graphics.VerifySampler(
						GLDevice,
						sampler,
			...
			for (int sampler = 0; sampler < modifiedVertexSamplers.Length; sampler += 1)
			{
				 ...
				 * -flibit
				 */
				XnaBackend.Graphics.VerifyVertexSampler(
					GLDevice,
					sampler,
		...
		{
			fixed (RhiVertexBufferBinding* b = &nativeBufferBindings[0])
			{
				for (int i = 0; i < vertexBufferCount; i += 1)
				{
				...
				XnaBackend.Graphics.ApplyVertexBufferBindings(
					GLDevice,
					(IntPtr) b,
					vertexBufferCount,
		...
		private unsafe void PrepareUserVertexBuffer(
			...
			if (len > userVertexBufferSize)
			{
				...
				userVertexBuffer = XnaBackend.Graphics.GenVertexBuffer(
					GLDevice,
					1,
			...
			fixed (RhiVertexBufferBinding* b = &nativeBufferBindings[0])
			{
				...
				XnaBackend.Graphics.ApplyVertexBufferBindings(GLDevice, (IntPtr) b, 1, 1, 0);
			}
			vertexBuffersUpdated = true;
		}
		...
		private void PrepareUserIndexBuffer(
			...
			if (len > userIndexBufferSize)
			{
				...
				userIndexBuffer = XnaBackend.Graphics.GenIndexBuffer(
					GLDevice,
					1,
					BufferUsage.WriteOnly,
					len
