	...
	internal class Texture2DReader : ContentTypeReader<Texture2D>
	{
		...
		protected internal override Texture2D Read(
			...
			for (int level = 0; level < levelCount; level += 1)
			{
				...
				if (levelData == null)
				{
					/* 5c-4: was FNA's internal MemoryStream.TryGetBuffer(out byte[]) extension — a shim
					 * for old Mono that stayed behind in FNA. The BCL's TryGetBuffer yields an
					 * ArraySegment, so this uses the same WPR helper 5c-1 introduced for the DDS loaders. */
					if (	ImageStreamHelper.TryGetBuffer(reader.BaseStream, out levelData)	)
					{
