...
**`WPR.XnaCompabilityPatch` no longer exists.** The follow-up audit above emptied it type by type,
and on 2026-08-29 the last two went and the project was deleted:

* the WP7 `GraphicsDeviceManager` override → `Src/Backends/WPR.Backend.FNA/Compat/`. It subclasses
  FNA's spine manager, and the backend was always a legal home for that — an option the earlier
  note here missed by only ever considering `WPR.Framework.Xna` (which *would* cycle) and
  concluding the type "goes when the spine goes". It did not have to wait.
* the `GraphicsDevice` / `GraphicsAdapter` display-mode overrides → `WPR.Framework.Xna/Compat/`,
  namespace `WPR.Xna.Compat` (beside the `WPR.Xna.Rhi` seam). These never touched FNA at all —
  they subclass the WPR-owned graphics types 5c-1 moved into that same assembly — so the only
  thing keeping them in a separate shim was inertia. They could *not* follow the manager into the
  backend: `WPR.Loader` keys them by `typeof(...)` for `MemberPatches`, and Loader cannot
  reference `WPR.Backend.FNA` because the backend already references Loader. Loader now
  references `WPR.Framework.Xna` directly instead.

Net effect: one fewer assembly, one fewer game-facing identity (games bound `WPR.XnaCompability`
before and bind `WPR.Framework.Xna` now — an assembly they already load), and the fitness baseline
lost an entry. `ApplicationPatcher.Version` 14 → 15 → 16 across the two steps, both
reinstall-forcing; a Version ≤15 install names an assembly that no longer ships.

Both moved types stay deliberately distinct from the real ones — that is the whole point of a
`MemberPatches` target. `GraphicsAdapter` in particular declares a literal `get_CurrentDisplayMode()`
*method* rather than a property, because `MemberPatches` matches the memberref signature and only
rewrites its declaring type; the member name has to survive verbatim.
