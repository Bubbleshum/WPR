...
| Tilt components | `TiltInputXnaComponent` / `TiltOverlayXnaComponent` → `WPR.Backend.FNA` | **S**, spine-blocked | They are FNA `GameComponent`s living in `WPR.Platform.Windows`, and they are the entire reason that head is still in `KnownBackendLeaks`. Cheap to relocate, but a `GameComponent` has to derive from *something* — the destination type only settles when the spine does. The heads then reach them through an orientation/overlay abstraction. |
