...
`WPR.WindowsCompability`, `WPR.SilverlightCompability`, `WPR.XnaCompability`,
`WPR.StandardCompability`. The patcher **rewrites** game IL to point here, so the
game never binds to these names on its own. Rename cost:
...
Every stage's exit gate is the same (full checklist in `docs/STAGE-GATE.md`):
**(a)** `WPR.UI.Desktop` builds for `net8.0-windows10.0.17763.0` and the Android
...
* Create `WPR.Abstractions` with the interface set: `IGraphicsContext`,
  ...
  **`IGameHost`** — a game-loop/lifecycle contract with **explicit teardown-ordering
  ...
  `ApplicationLaunch.cs` has no equivalent otherwise — see `docs/STAGE5-SIZING.md`
  Risk #1).
...
2026-08-06, then un-consolidated the same day to drop the shims.) Fitness baseline: `Microsoft.Devices.Sensors`
(FNA leak, de-FNA'd at Stage 5d); `System.Device` is clean. The empty-orphan `Microsoft.Devices` project
...
**Sized XL — see `docs/STAGE5-SIZING.md` for the per-project audit.** The cost is
...
4. **Green-gate smoke pair** — **Minesweeper** and **MonstaFish**. Both must reach
   gameplay after reinstall at every stage's exit (see `docs/STAGE-GATE.md`).
